...
#define TIMEOUT 15  //超时，单位s，代表一组中的所有ack都正确接收
...
void ackhandler(unsigned int a)
{
	...
	case SLOWSTART:
		if ((index + seqnumber - curack) % seqnumber < minwindow(cwnd, WINDOWSIZE))
		{
			...
			ack[index % WINDOWSIZE] = 1;
			totalack++;

			preack = curack;
			if (!srFlag)
				curack = index; //相对ack计算
			else
				globalack = index % WINDOWSIZE;
			if (curack - preack > 1)
			{
				srFlag = true;
			}
		...
		if (pkt->tag == 0)
		{
			...
			while (runFlag)
			{
				...
				case 2:
					...
					// 	sendto(sockServer, (char*)pkt, BUFFER, 0, (SOCKADDR*)&addrClient, sizeof(SOCKADDR));
					// 	cout << "[Send seq " << curseq << " 的数据包]" << endl << endl;
					// 	ack[curseq % WINDOWSIZE] = 2;
					// 	++curseq;
					// 	curseq %= seqnumber;
					// }

					while((curseq + seqnumber - curack) % seqnumber < minwindow(cwnd, WINDOWSIZE) && sendwindow>0)
					{
						ZeroMemory(buffer[curseq % WINDOWSIZE], BUFFER);
							pkt->init_packet();
							if (length1 >= 1024)
